"""
agents.py — The 3 Independent Agent Definitions
=================================================
Every agent uses an OpenAI-compatible chat completions client.
Model names and API credentials are read from .env - no hardcoded values.

Agent roster:
  1. RetrieverAgent - grades chunks, rewrites queries
  2. AnswererAgent  - generates cited answers
  3. CriticAgent    - fact-checks the answer
"""
...
class BaseAgent:
    ...
    def _call_json(self, user_message: str) -> dict:
        ...
        If parsing fails, falls back to a retry with an explicit instruction.
        """
        try:
            response = client.chat.completions.create(
                model=self.model,
...
class RetrieverAgent(BaseAgent):
    """
    Runs on every query, possibly twice.
    ...
    def run(self, query: str) -> dict:
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            ...
            if rewritten and attempt < MAX_ATTEMPTS:
                print(f"  [RetrieverAgent] score={avg_score:.2f} - rewriting query")
                print(f"     Original : \"{current_query}\"")
                print(f"     Rewritten: \"{rewritten}\"")
                current_query = rewritten
            else:
                print(f"  [RetrieverAgent] max attempts - using best context (score={avg_score:.2f})")
                return best
...
class AnswererAgent(BaseAgent):
    ...
    Uses ONLY the retrieved chunks - no outside knowledge.
    """
