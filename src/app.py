...
MODEL_RETRIEVER = os.getenv("MODEL_RETRIEVER", "unset")
MODEL_ANSWERER  = os.getenv("MODEL_ANSWERER", "unset")
MODEL_CRITIC    = os.getenv("MODEL_CRITIC", "unset")

st.set_page_config(page_title="Multi-Agent RAG", page_icon="🤖", layout="wide")
st.title("🤖 Multi-Agent Self-Correcting RAG")
st.caption("3 independent agents: Retriever -> Answerer -> Critic")
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**Agents**")
    st.markdown(f"RetrieverAgent — {MODEL_RETRIEVER}")
    st.markdown(f"AnswererAgent — {MODEL_ANSWERER}")
    st.markdown(f"CriticAgent — {MODEL_CRITIC}")
    st.divider()
